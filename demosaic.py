...
class Demosaic:
    ...
    def _demosaic_with_rawpy(self, bayer_array: np.ndarray, algorithm: str) -> np.ndarray:
        ...
        # =================================================================
        # 🌟  零损耗数据流：临时 16-bit 桥接
        # LibRaw 不接受浮点矩阵直接处理，所以我们需要临时将 float32 转为 uint16
        # =================================================================
        print("    [Info] Rawpy 后端: 执行临时 16-bit 高精度量化转换...")
        bayer_uint16 = np.clip(np.round(bayer_array * 65535.0), 0, 65535).astype(np.uint16)
        ...
        # =================================================================
        BLACK_LEVEL = 0
        WHITE_LEVEL = 65535 # [修改] 既然已经映射到 16-bit，满量程就是 65535
        ...
        with io.BytesIO() as tiff_buffer:
            ...
            with rawpy.imread(tiff_buffer) as raw:
                ...
                rgb_image = raw.postprocess(
                    ...
                    # --- 输出格式 ---
                    output_bps=16 # [修改] 强制输出 16-bit，以保留最大精度
                )
        
        # =================================================================
        # 🌟 [修改] 零损耗数据流：恢复 float32 输出
        # =================================================================
    ...
    def _demosaic_with_colour(self, bayer_array: np.ndarray, algorithm: str) -> np.ndarray:
        ...
        # =================================================================
        # 🌟 [修改] 零损耗数据流：完美契合
        # 数据本身已经是 float32 [0.0, 1.0]，为了该库更高精度转为 float64
        # =================================================================
        bayer_float = bayer_array.astype(np.float64)
        ...
        # =================================================================
        # 🌟 [修改] 零损耗数据流：直接返回 Float32，无需转回整型
        # =================================================================
        return np.clip(rgb_float, 0.0, 1.0).astype(np.float32)
    ...
    def _demosaic_with_cv2(self, bayer_array: np.ndarray, algorithm: str) -> np.ndarray:
        ...
        # =================================================================
        # 🌟 [修改] 零损耗数据流：临时 16-bit 桥接
        # OpenCV 的 VNG 等算法对浮点数支持不佳，为安全起见临时转为 uint16
        # =================================================================
        print("    [Info] OpenCV 后端: 执行临时 16-bit 高精度量化转换...")
        bayer_uint16 = np.clip(np.round(bayer_array * 65535.0), 0, 65535).astype(np.uint16)

        if algorithm.upper() == 'CV_VNG':
             # VNG 算法需要 3 通道输出
             rgb_uint16 = cv2.cvtColor(bayer_uint16, cv_pattern.replace("RGB", "RGB_VNG"))
        else:
             # 默认 'CV'
             rgb_uint16 = cv2.cvtColor(bayer_uint16, cv_pattern)
             
        # 恢复 float32 输出
        return rgb_uint16.astype(np.float32) / 65535.0
    ...
    def execute(self, bayer_array: np.ndarray, algorithm: str = 'AHD') -> np.ndarray:
        ...
        Args:
            bayer_array (np.ndarray): 2D Bayer 数组 (来自 RawDenoise, float32)
            algorithm (str):
        ...
        # =================================================================
        # 🌟 [修改] 零损耗数据流：解除严格的数据类型壁垒限制
        # 不再强制要求 bayer_array.dtype == self.dtype，只要是 float 就可以流转
        # =================================================================
        if not np.issubdtype(bayer_array.dtype, np.floating):
            print("警告: Demosaic 期望输入 Float32 格式。正在强制转换...")
            bayer_array = bayer_array.astype(np.float32)
