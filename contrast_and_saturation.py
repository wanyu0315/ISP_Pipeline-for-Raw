...
class ContrastSaturation:
    ...
    输入为 np.float32 YUV图像 (支持 HDR 范围)，
    输出为 np.float32 YUV图像 (不强制截断)。
    """
    ...
    def execute(self, yuv_image: np.ndarray, 
        ...
        # ===================================================================
        # 🌟 [修改] 零损耗数据流：只校验类型，不因 max>1.0 而除以 65535
        # ===================================================================
        if not np.issubdtype(yuv_image.dtype, np.floating):
            print(f"  [警告] ContrastSaturation 接收到 {yuv_image.dtype}，强制转为 float32。")
        ...
        # ===================================================================
        # 🌟 [修改] 零损耗数据流：移除最终的 np.clip，保留管线级的 HDR 能力
        # ===================================================================
        return result
    ...
    def _histogram_equalization(self, channel: np.ndarray, **kwargs) -> np.ndarray:
        ...
        [!] 此操作会导致 HDR 截断
        """
        print("  [!] 警告: 直方图均衡化将截断 HDR 高光，强制在 8-bit 下运行。")
        # 必须先 clip 防止 HDR 翻转
        channel_uint8 = np.clip(np.round(channel * 255.0), 0, 255).astype(np.uint8)
    ...
    def _clahe(self, channel: np.ndarray, clip_limit: float = 2.0, 
        ...
        [!] 此操作会导致 HDR 截断
        """
        print("  [!] 警告: CLAHE 将截断 HDR 高光，强制在 8-bit 下运行。")
    ...
    def _gamma_contrast(self, channel: np.ndarray, gamma: float, **kwargs) -> np.ndarray:
        ...
        # 防止负数被 np.power 产生 NaN
        channel_safe = np.clip(channel, 0.0, None)
        adjusted = np.power(channel_safe, 1.0 / gamma)
        return adjusted
    ...
    def _sigmoid_contrast(self, channel: np.ndarray, strength: float = 5.0, 
        ...
        range_val = adj_max - adj_min
        if range_val > 1e-6:
            adjusted = (adjusted - adj_min) / range_val
        else:
            adjusted = np.full_like(adjusted, adj_min)
            
        # [保留Clip] 由于Sigmoid自身的归一化逻辑，此处clip是合理的
    ...
    def _linear_saturation(self, yuv_image: np.ndarray, factor: float, **kwargs) -> np.ndarray:
        ...
        result[:, :, 1] = u_centered * factor + 0.5
        result[:, :, 2] = v_centered * factor + 0.5
    ...
    def _vibrance(self, yuv_image: np.ndarray, factor: float, 
        ...
        sat_norm = current_saturation * 1.414 
        ...
        result[:, :, 1] = u_centered * final_factor + 0.5
        result[:, :, 2] = v_centered * final_factor + 0.5
    ...
    def _hsv_saturation(self, yuv_image: np.ndarray, factor: float, **kwargs) -> np.ndarray:
        """
        HSV空间饱和度调整 - 需经过uint8中转
        [!] 此操作将截断 HDR 数据
        """
        print("  [!] 警告: HSV 饱和度调整将截断 HDR 高光。")
        rgb_image_float = self._yuv_to_rgb(yuv_image)
        
        # 防止越界翻转
        rgb_uint8 = np.clip(np.round(rgb_image_float * 255.0), 0, 255).astype(np.uint8)
    ...
    def _selective_saturation(self, yuv_image: np.ndarray, factor: float,
        ...
        [!] 此操作将截断 HDR 数据
        """
        print("  [!] 警告: Selective 饱和度调整将截断 HDR 高光。")
        rgb_image_float = self._yuv_to_rgb(yuv_image)
    ...
    def _rgb_to_yuv(self, rgb_float: np.ndarray) -> np.ndarray:
        ...
        return np.stack((y, u + 0.5, v + 0.5), axis=-1)
