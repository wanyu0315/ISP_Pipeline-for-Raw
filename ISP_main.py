...
def main_batch():
    ...
    ROOT_INPUT_DIR = 'ISPpipline/raw_data/baseenvironment_rawframe'

    # 存放所有处理后 PNG 帧的根目录
    ROOT_OUTPUT_FRAME_DIR = '/home/lizize/pyVHR_for_ISP/ISPpipline/isp_output_frame/sensitivity_analysis/baseenv/gamma/0.8'
    
    # 存放最终输出视频和 JSON 参数文件的根目录
    ROOT_OUTPUT_VIDEO_DIR = 'Data_for_pyVHR/isp_output_Video/sensitivity_analysis/baseenv/gamma/0.8'
    ...
    processing_params = {
        ...
        'gammacorrection': {'gamma': 0.8, 'method': 'simple'},
    ...
    for input_folder in raw_folders:
        ...
        if not skip_processing:
            ...
            # 探针数据保存路径
            probe_save_dir = os.path.join("isp_debug_probes/sensitivity_analysis/baseenv/gamma/0.8", video_name)
            ...
            my_isp = ISPPipeline(modules=[
                ...
                PipelineProbe(probe_name="1_PrevGamma", 
                              save_dir=probe_save_dir,
                              auto_detect_roi=True,
                              start_frame=PROBE_START, max_frames=PROBE_MAX),
                              
                GammaCorrection(),                # 伽马校正
                
                PipelineProbe(probe_name="2_PostGamma", 
                              save_dir=probe_save_dir,
                              auto_detect_roi=True,
                              start_frame=PROBE_START, max_frames=PROBE_MAX),
...
# #     # 存放所有处理后 PNG 帧的根目录
# #     ROOT_OUTPUT_FRAME_DIR = '/home/lizize/pyVHR_for_ISP/ISPpipline/isp_output_frame/sensitivity_analysis/baseenv/gamma/1.0'
    
# #     # 存放最终输出视频和 JSON 参数文件的根目录
# #     ROOT_OUTPUT_VIDEO_DIR = 'Data_for_pyVHR/isp_output_Video/sensitivity_analysis/baseenv/gamma/1.0'
...
# #         video_name = os.path.basename(input_folder) # 例如: "raw_lzz"
# #         print(f"\n" + "="*50)
# #         print(f"▶ 正在处理视频序列: {video_name}")
# #         print("="*50)

# #         # --- 为当前处理的图像和生成的视频动态生成输出路径 ---
# #         output_folder = os.path.join(ROOT_OUTPUT_FRAME_DIR, video_name)
# #         output_video_path = os.path.join(ROOT_OUTPUT_VIDEO_DIR, f"{video_name}_output.mkv")
