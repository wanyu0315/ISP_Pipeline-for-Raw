...
class Denoise:
    ...
    所有方法均假设输入为 np.float32 YUV图像 (支持 >1.0 的 HDR 范围)，
    输出也为 np.float32 YUV图像，全程不截断。
    """
    ...
    def execute(self, yuv_image: np.ndarray, algorithm: str = 'bilateral', **kwargs) -> np.ndarray:
        ...
        Args:
            yuv_image: 输入的YUV图像 (np.float32, 包含 HDR 数据)
            algorithm: 降噪算法选择
                'gaussian': 高斯滤波 (float32 兼容)
                'bilateral': 双边滤波 (保边降噪, 已升级为 float32 兼容)
                'nlm': 非局部均值降噪 (需转uint8)
                'nlm_fast': 快速非局部均值 (需转uint8)
                'median': 中值滤波 (ksize<=5 时 float32 兼容)
                'wavelet': 小波降噪 (float32 兼容)
                'anisotropic': 各向异性扩散 (float32 兼容)
            **kwargs: 算法特定参数
        
        Returns:
            降噪后的YUV图像 (np.float32)
        """
        print(f"Executing Denoising using algorithm: {algorithm}")
        
        # ===================================================================
        # 🌟 [修改] 零损耗数据流：信任上游数据，不再错误地将 HDR > 1.0 误判为 16-bit
        # ===================================================================
        if not np.issubdtype(yuv_image.dtype, np.floating):
            print(f"  [警告] Denoise 模块期望接收 float32，当前为 {yuv_image.dtype}。正在转换...")
    ...
    def _gaussian_denoise(self, image: np.ndarray, sigma: float = 1.0, 
        ...
        # [修改] 移除 np.clip，保留浮点原始输出
        return denoised_image
    
    def _bilateral_denoise(self, image: np.ndarray, d: int = 9, 
                           sigma_color: float = 75, sigma_space: float = 75) -> np.ndarray:
        """
        双边滤波 - 保边降噪，适合保留细节 (已升级为完全 float32 支持)
        Args：
            d: 滤波器直径
            sigma_color: 颜色空间标准差 (需根据 [0, 1] 范围进行缩放)
            sigma_space: 坐标空间标准差
        """
        # ===================================================================
        # 🌟 [修改] 零损耗数据流：OpenCV 的 bilateralFilter 完美支持 float32
        # 我们只需将针对 0-255 设计的 sigma_color 缩放到 0-1 尺度即可
        # ===================================================================
        sigma_color_scaled = sigma_color / 255.0
        
        denoised_float = cv2.bilateralFilter(image, d, sigma_color_scaled, sigma_space)
    ...
    def _nlm_denoise(self, image: np.ndarray, h: float = 10, 
        ...
        """
        非局部均值降噪 - 高质量但较慢(需临时转uint8)

        注意: OpenCV 的 NLM 实现 (尤其是彩色) 严格期望 uint8 输入。
        """
        print("  [!] 警告: NLM 降噪强制使用 8-bit 处理，将导致 HDR 高光截断和局部量化损失。")
        ## --- 兼容性核心：执行局部 float32 -> uint8 -> float32 转换 ---
        # 1. 转换为 [0, 255] uint8，增加 clip 保护防止 HDR 溢出导致越界翻转
        img_uint8 = np.clip(np.round(image * 255.0), 0, 255).astype(np.uint8)
    ...
    def _median_denoise(self, image: np.ndarray, ksize: int = 5) -> np.ndarray:
        """
        中值滤波 -  对椒盐噪声效果好 (智能支持 float32)
        
        注意: cv2.medianBlur 对 float32 的支持极其严苛，ksize 必须且只能是 3 或 5。
        """
        if ksize % 2 == 0:
        ...
        # ===================================================================
        if ksize <= 5:
            # OpenCV 允许 ksize 为 3 或 5 的 float32 直接处理！
            return cv2.medianBlur(image, ksize)
        else:
            print(f"  [!] 警告: cv2.medianBlur 在 float32 下不支持 ksize={ksize}。临时降级为 8-bit 处理！")
            img_uint8 = np.clip(np.round(image * 255.0), 0, 255).astype(np.uint8)
