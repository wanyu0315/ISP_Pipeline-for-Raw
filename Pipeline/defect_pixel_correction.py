...
class DefectPixelCorrection:
    ...
    def _detect_defects(self, raw_data: np.ndarray,
        ...
        threshold = threshold_factor * (local_std + 1e-5)  # +1避免除零 ( 注意：在 [0.0, 1.0] 空间建议改成 + 1e-5)
        
        defect_map = deviation > threshold
        
        # ===================================================================
        # 🌟 [修改] 零损耗数据流：动态获取浮点域的最大值
        # 避免浮点数输入导致 np.iinfo 崩溃
        # ===================================================================
    ...
    def _mean_correction(self, raw_data: np.ndarray,
        ...
        for y, x in zip(*defect_coords):
            ...
            if len(valid_pixels) > 0:
                # ===================================================================
                # 🌟 [修改] 零损耗数据流：去除了 int() 强制转换
                # 保留浮点数的高精度均值，避免吃掉微弱信号
                # ===================================================================
    ...
    def _gradient_correction(self, raw_data: np.ndarray,
        ...
        # ===================================================================
        # 🌟 [修改] 零损耗数据流：直接返回修正后的 float32 数据
        # 移除了 redundant 的 .astype(raw_data.dtype)，严格保留浮点
        # ===================================================================
        return corrected
    
    def create_defect_map_from_dark_frame(self, dark_frame: np.ndarray,
                                         threshold: float = None) -> np.ndarray:  # [修改] 阈值支持 float
        """
        从暗场图像创建坏点图
        ...
        if threshold is None:
            # ===================================================================
            # 🌟 [修改] 零损耗数据流：移除了 int() 强制转换
            # 允许使用高精度浮点阈值，这对于归一化的暗场极其重要
            # ===================================================================
            threshold = np.mean(dark_frame) + 3 * np.std(dark_frame)
