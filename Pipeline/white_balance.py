...
class WhiteBalanceRaw:
    ...
    def execute(self, image: np.ndarray, algorithm: str = 'gray_world_green', 
        ...
        Args:
            image: 输入Raw Bayer数据 (2D数组, HxW, float32)
            algorithm: 白平衡算法
                - 'gray_world': 标准灰度世界
    ...
    def _gray_world_green_raw(self, image: np.ndarray, bayer_pattern: str) -> np.ndarray:
        ...
        # ===================================================================
        # 🌟 [修改] 零损耗数据流：直接返回 Float32，删除了 overflow ratio 的检测
        # (因为在 HDR 浮点空间，大于 1.0 是合法且有益的)
        # ===================================================================
        return img_float
    ...
    def compute_wb_gains(self, image: np.ndarray, bayer_pattern: str,
        ...
        gains = {
            'R': float(np.clip(gain_r, *self.gain_limits)),
            'Gr': float(np.clip(gain_gr, *self.gain_limits)),
            'Gb': float(np.clip(gain_gb, *self.gain_limits)),
            'B': float(np.clip(gain_b, *self.gain_limits))
        }
