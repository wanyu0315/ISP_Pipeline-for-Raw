...
class BlackLevelCorrection:
    ...
    def execute(self, raw_data: np.ndarray, 
                black_level: float = None,  # [修改] 允许传入浮点数
        ...
        # [修改] 识别当前数据是否为 float32 (已经历过 RawLoader 的提权)
        is_float = np.issubdtype(raw_data.dtype, np.floating)
        ...
        if auto_detect:
            ...
            # [修改] 防御机制：如果外部强行传了整数参数（如 2048），将其归一化到 [0.0, 1.0]
            if is_float and black_level > 1.0:
                black_level = black_level / 65535.0  # 假设用户传入的是 16bit 阈值
            print(f"  Manual black level: {black_level}")
        
        # 2. 执行校正 (全程 float32 运算)
        ...
        # 4. 转换回原类型 
        # ===================================================================
        # 🌟 [修改] 零损耗数据流：阶段二 - 全程浮点漫游
        # 绝不执行 .astype(raw_data.dtype) 转换回整型，保持高精度浮点
        # ===================================================================
        return corrected
    
    def _estimate_black_level(self, raw_data: np.ndarray) -> float:
        """
        根据位深估计典型黑电平值
        """
        # [修改] 如果数据已经是浮点数，np.iinfo 会报错。直接返回典型的 16-bit 归一化黑电平
        if np.issubdtype(raw_data.dtype, np.floating):
            return 2048.0 / 65535.0
    ...
    def _detect_black_level(self, raw_data: np.ndarray, 
        ...
        """
        # [修改] 去掉 int() 强转，保留微小的浮点波动精度
        black_level = float(np.percentile(raw_data, percentile))
...
class LensShadingCorrection:
    ...
    def _apply_gain_map(self, raw_data: np.ndarray, 
        ...
        # ===================================================================
        # 🌟 [修改] 零损耗数据流：阶段二 - 全程浮点漫游
        # 删除了 np.clip 和 .astype()
        # 允许像素值在 float32 空间中临时超过 1.0 (保留内部的高动态范围 HDR)
        # ===================================================================
        return corrected
